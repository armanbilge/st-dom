package dom.std.global

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* This class was inferred from a value with a constructor. In rare cases (like HTMLElement in the DOM) it might not work as you expect. */
@JSGlobal("MSMediaKeyError")
@js.native
class MSMediaKeyError ()
  extends StObject
     with dom.std.MSMediaKeyError
object MSMediaKeyError {
  
  @JSGlobal("MSMediaKeyError.MS_MEDIA_KEYERR_CLIENT")
  @js.native
  val MS_MEDIA_KEYERR_CLIENT: Double = js.native
  
  @JSGlobal("MSMediaKeyError.MS_MEDIA_KEYERR_DOMAIN")
  @js.native
  val MS_MEDIA_KEYERR_DOMAIN: Double = js.native
  
  @JSGlobal("MSMediaKeyError.MS_MEDIA_KEYERR_HARDWARECHANGE")
  @js.native
  val MS_MEDIA_KEYERR_HARDWARECHANGE: Double = js.native
  
  @JSGlobal("MSMediaKeyError.MS_MEDIA_KEYERR_OUTPUT")
  @js.native
  val MS_MEDIA_KEYERR_OUTPUT: Double = js.native
  
  @JSGlobal("MSMediaKeyError.MS_MEDIA_KEYERR_SERVICE")
  @js.native
  val MS_MEDIA_KEYERR_SERVICE: Double = js.native
  
  @JSGlobal("MSMediaKeyError.MS_MEDIA_KEYERR_UNKNOWN")
  @js.native
  val MS_MEDIA_KEYERR_UNKNOWN: Double = js.native
}
